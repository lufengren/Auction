...
export class ProductDetailComponent implements OnInit {
  ...
  submitNewComment() {
    ...
    const newProductComment = new Comment(uuid, this.productId, this.newComment, new Date().toDateString(), "Joe", this.newScore);
    this.comments.unshift(newProductComment);
    const sumOfScore = this.comments.reduce((sum, comment) => sum += comment.rating, 0);
